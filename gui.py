...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        wx.Frame.__init__(self, parent, title=title, size=(
            270, 340), style=wx.DEFAULT_FRAME_STYLE & (~wx.RESIZE_BORDER))
        panel = wx.Panel(self)

        # GENERATE BUTTON
        button = wx.Button(panel, wx.ID_ANY, label="Generate",
                           pos=(70, 270), size=(120, 40))
        self.Bind(wx.EVT_BUTTON, self.OnClick, button)

        # BANDTYPE
        ftype_txt = wx.StaticText(panel, label="Band type:", pos=(150, 140))
        btype_list = ["Lowpass", "Highpass", "Bandpass", "Bandstop"]
        self.btype_listbox = wx.ListBox(
            panel, wx.ID_ANY, (150, 155), (100, 100), btype_list, wx.LB_SINGLE)
        ...
        self.ftype_listbox = wx.ListBox(
            panel, wx.ID_ANY, (150, 35), (100, 100), ftype_list, wx.LB_SINGLE)
        ...
        self.gpass_spinner = wx.SpinCtrl(
            panel, wx.ID_ANY, "", (60, 105), (70, 20))
        ...
        self.gstop_spinner = wx.SpinCtrl(
            panel, wx.ID_ANY, "", (60, 135), (70, 20))
        ...
        self.R1_spinner = wx.SpinCtrl(
            panel, wx.ID_ANY, "", (60, 165), (70, 20))
        ...
        self.R2_spinner = wx.SpinCtrl(
            panel, wx.ID_ANY, "", (60, 195), (70, 20))
    ...
    def get_ftype(self, event):
        ...
        types_dict = {"Butterworth": "butter", "Chebyshev I": "cheby1",
                      "Chebyshev II": "cheby2", "Cauer": "ellip"}
    ...
    def validation(self):
        if self.fp == self.fs:
            dlg = wx.MessageDialog(
                None, "Passband and stopband corner frequencies must be different!", "Warning", wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            return False
        elif self.gpass == self.gstop:
            dlg = wx.MessageDialog(
                None, "Passband ripple and stopband attenuation must be different!", "Warning", wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            return False
        else:
            return True
    ...
    def OnClick(self, event):
        ...
        pyplot.close("all")
        filter_ = Filter(self.fp, self.fs, self.gpass,
                         self.gstop, ftype=self.ftype, btype=self.btype)
        ...
        if self.ftype == "ellip":
            ...
            ladder = LCladder(self.R1, self.R2, self.fp, self.fs,
                              self.gpass, self.gstop, ftype=self.ftype, btype=self.btype)
